---
description: BFF-часть пайплайна bff-pipeline-v2 — этапы 0–4 (preflight → schema → datasource → resolver → e2e) через детерминированный Node.js runner. Триггеры: «новая graphql фича в bff», «добавь query/mutation в bff-mls/bff-admin», «прокинь эндпоинт», «bff-pipeline bff».
mode: all
---
...
| 1 | schema | `bff-schema-create-v2` | `verify --stage 1` |
| 2 | datasource | `bff-datasource-create-v2` | `verify --stage 2` |
| 3 | resolver | `bff-resolver-create-v2` | `verify --stage 3` |
| 4 | e2e | `bff-e2e-verify-v2` | `verify --stage 4` |
...
После **PASS этапа 4** сообщи: «Этапы 0–4 PASS. BFF-часть готова — дальше клиентская часть (этапы 5–6), агент `bff-pipeline-client`» и предложи вызвать его (`@bff-pipeline-client`).
